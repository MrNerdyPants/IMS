...
@Component({
  selector: 'app-add-checklist-category',
  templateUrl: './add-checklist-category.component.html',
  styleUrls: ['./add-checklist-category.component.scss']
})
export class AddChecklistCategoryComponent {
  addForm: any;
  loading = false;
  states: any;
  ...
  onSubmit(): void {
    if (this.addForm.valid) {
      ...
      this.genericService.saveData('settings/save-checklist-category', obj)
        .subscribe(
          (data: any) => {
            if (data.code === 200) {
