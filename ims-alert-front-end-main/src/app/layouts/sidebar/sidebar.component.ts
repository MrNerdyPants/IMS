...
export class SidebarComponent implements OnInit {
  ...
  constructor(private router: Router, public translate: TranslateService, private local: LocalService) {
  ...
  ngOnInit(): void {
    ...
    if (isUserAuthenticate) {
      ...
      for (let right of userRights) {
        var childrens = this.sortRightChildren(right.children, right.id);
        if (childrens?.length > 0) {
          ...
          this.menuItems.push(parentObject);
        }
      }
    } else {
  ...
  sortRights(rights: any): any {
    return rights?.sort((r1: any, r2: any) => {
      if (r1.sort > r2.sort) {
        return 1;
  ...
  sortRightChildren(childern: any, parentId: number): any {
    let childernArray = this.sortRights(childern);
    var sortedChildren = [];
    for (let child of childernArray) {
      if (child.displayInd === 'Y') {
        var childObject = {
          id: child.id,
          name: child.name,
          url: child.url,
        ...
        sortedChildren.push(childObject);
      }
    }
